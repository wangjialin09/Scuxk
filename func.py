...
def get_captcha(url, session):
    ...
    while flag:
        ...
        # print(content)
        _codes = requests.post("https://capocrapi.wjlo.cc/ocr/file/text", files={'image': content})
        _code = _codes.text
...
def get_already_course(session):
    ...
    try:
        ...
        for each in json.loads(response)['xkxx'][0]:
            # print(json.loads(response)['xkxx'][0][each]['courseName'])
